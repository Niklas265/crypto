...
string Toolbox::readFromStream(istream &strm, bool skipNewlines) {
    string line;
    string data;
    char c;
...
bool Toolbox::readFromFile(const string &filename, string &output, bool skipNewlines) {
    ifstream strm;

    strm.open(filename);
    if (strm.is_open()) {
        output = Toolbox::readFromStream(strm, skipNewlines);
        strm.close();
        return true;
    } else {
        return false;
